...
bottoken = 'NzA3NTM1MjQ5MTU1ODE3NTA0.XrKUOg.qdPPgfAMTmtZizk_yLMa7yDvbvs'
...
async def on_ready(): # on_ready() event : when the bot has finised logging in and setting things up
    await client.change_presence(status=discord.Status.online, activity=discord.Game("롤 전적 검색중 [ MFP ]"))
...
async def on_message(message): # on_message() event : when the bot has recieved a message
    ...
    if message.content.startswith("!help") or message.content.startswith("!도움말"):
        embed = discord.Embed(title="명령어 사용방법!", description="!롤전적+이름 이런식으로 입력해주시면 됩니다", color=0x5CD1E5)
